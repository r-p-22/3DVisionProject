...
using namespace Eigen;
   ...
   struct CameraMatrix
   {
         ...
         void setIntrinsic(Matrix3d const& K) { _K = K; this->updateCachedValues(true, false); }
         void scale(double scale_x, double scale_y)
         {
             _K(0,0) *= scale_x;
             _K(0,1) *= scale_x;
             _K(0,2) = (_K(0,2) + 0.5)*scale_x - 0.5;
             _K(1,1) *= scale_y;
             _K(1,2) = (_K(1,2) + 0.5)*scale_y -  0.5;
             this->updateCachedValues(true,false);
         }

         void setRotation(Matrix3d const& R) { _R = R; this->updateCachedValues(false, true); }
         ...
         Matrix3d const& getIntrinsic()   const { return _K; }
         Matrix3d const& getRotation()    const { return _R; }
         Vector3d   const& getTranslation() const { return _T; }

         Matrix<double,3,4>  getExtrinsic() const
         {
         ...
         Matrix<double,3,4>  getOrientation() const
         {
            Matrix<double,3,4>  RT;
         ...
         Matrix<double,3,4>  getProjection() const
         {
            Matrix<double,3,4>  const RT = this->getOrientation();
            return _K * RT;
         }
         ...
         Vector3d unprojectPixel(Vector2f const &p, double depth = 1) const
         {
            Vector3d pp;
         ...
         Vector3d unprojectNormalizedPixel(Vector2f const &p, double depth = 1) const
         {
            Vector3d pp;
            pp(0) = p(0); pp(1) = p(1); pp(2) = 1.0;
            Vector3d ray =  pp;
         ...
         Vector3d intersectRayWithPlane( const Vector4d &plane, int x, int y ) const
         {
             ...
             double rho = (-_center.dot(plane.head(3)) - plane(3)) /
                     ray.dot(plane.head(3));
         ...
         Vector2d transformPointIntoNormalizedCoordinate(Vector2d const& p) const
         {
            return Vector2d(_K(0,0) * p(0) + _K(0,1) * p(1) + _K(0,2),_K(1,1) * p(1) + _K(1,2));
         }
         Vector2d transformPointFromNormalizedCoordinate(Vector2d const& p) const
         {
            return Vector2d(_invK(0,0) * p(0) + _invK(0,1) * p(1) + _invK(0,2),_invK(1,1) * p(1) + _invK(1,2));
         }
